...
- ratings → media rating engine
- tags → polymorphic (target_type + target_id)
- follows / follow_requests / user_blocks → relationship graph
- conversations / messages → DM & group chat system

Important behavior:
- tags, likes, views are polymorphic
- conversations store participant_ids as UUID[]
- many operations rely on SECURITY DEFINER RPC functions
- delete_*_cascade functions handle cleanup logic
- RLS is enabled on multiple tables

Common failure areas:
- RLS blocking selects/inserts
- triggers modifying data unexpectedly
- cascade functions overlapping with FK constraints


## Full Schema Dump (Authoritative)
